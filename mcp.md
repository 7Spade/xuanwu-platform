  ...
  "mcpServers": {
    ...
    "firebase-mcp-server": {
      ...
      "env": {
        ...
        "SERVICE_ACCOUNT_KEY_PATH": "$COPILOT_MCP_FIREBASE_SERVICE_ACCOUNT_KEY_PATH"
      },
      "tools": ["*"]
    }
  }
...
> - `firebase-mcp-server` 已綁定至 Firebase 專案 `xuanwu-i-00708880-4e2d8`。Admin SDK 功能（Firestore write、Auth 管理等）需要 Service Account：在 GitHub 或 Copilot 環境的 Secrets 設定 `COPILOT_MCP_FIREBASE_SERVICE_ACCOUNT_KEY_PATH`（指向 Service Account JSON 檔案的路徑）。唯讀 / 結構查詢只需 `FIREBASE_PROJECT_ID`，本機開發亦可透過 `firebase login` 取得 ADC 憑證。
