...
| BDY-001 | 邊界 | 🔴 高 | `collaboration.module` Presentation 直接實例化 infra repository |
| BDY-002 | 邊界 | 🔴 高 | `fork.module` Presentation 直接實例化 infra repository |
| BDY-003 | 邊界 | 🔴 高 | `social.module` Presentation 三層直通（infra + use-case + domain VO） |
| BDY-004 | 邊界 | 🔴 高 | `workforce.module` Presentation 直接實例化 infra repository |
| BDY-005 | 邊界 | 🟡 中 | `collaboration.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-006 | 邊界 | 🟡 中 | `fork.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-007 | 邊界 | 🟡 中 | `social.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-008 | 邊界 | 🟡 中 | `workforce.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-009 | 邊界 | 🟡 中 | `achievement.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-010 | 邊界 | 🟡 中 | `notification.module` `_components` 繞過 `index.ts` 直通 `core/_use-cases` |
| BDY-011 | 邊界 | 🟡 中 | `social.module` `_components` 直通 Domain layer value-object |
| BDY-012 | 邊界 | 🟡 中 | `fork.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
| BDY-013 | 邊界 | 🟡 中 | `workforce.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
| BDY-014 | 邊界 | 🟡 中 | `social.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
| BDY-015 | 邊界 | 🟡 中 | `achievement.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
| BDY-016 | 邊界 | 🟡 中 | `notification.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
| BDY-017 | 邊界 | 🟡 中 | `collaboration.module` `index.ts` 直接 re-export `_use-cases`，facade 未就位 |
...
## Part K: Module Boundary Violations / 模組邊界違規
